...
- **function**: account-archived
- **source_state**: `ACCOUNT_ACTIVATED`
- **target_state**: `ACCOUNT_ARCHIVED`
- **event_type**: `ACCOUNT_ARCHIVED`
- **event_format**: [application/hal+json](http://stateless.co/hal_specification.html)

**Description**:

This event handler is triggered in response to an _Account_ resource transitioning from an `ACCOUNT_ACTIVATED` status to an `ACCOUNT_ARCHIVED` status. The function confirms the target state of the _Account_ API resource that is linked within the payload of the supplied _AccountEvent_ resource.
